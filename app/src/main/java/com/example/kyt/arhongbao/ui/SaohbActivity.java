...
public class SaohbActivity extends AppCompatActivity implements SurfaceHolder.Callback,SensorEventListener, android.hardware.Camera.PictureCallback{
    private Button shb;
    ...
    private final int START_SHAKE=0x13;
    private final int START_TAKEPIC=0x14;
    ...
    Handler mHandler=new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                ...
                case START_TAKEPIC:
                    //自动拍照
                    camera.takePicture(null,null,null,SaohbActivity.this);
                    break;
            }
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mSensorManager==null){
            Toast.makeText(SaohbActivity.this,"设别不支持传感器!",Toast.LENGTH_SHORT).show();
        }
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER){
            ...
            if ((Math.abs(event.values[0] - gravity[0]) < 0.02 && Math.abs(event.values[1] - gravity[1]) < 0.02 && Math.abs(event.values[2] - gravity[2]) < 0.02)) {
                mSensorManager.unregisterListener(this);
                Toast.makeText(SaohbActivity.this,"静止，拍摄",Toast.LENGTH_SHORT).show();
                Log.d("MM", "没有晃动");
